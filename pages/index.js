...
export default function Home() {
  ...
  return !authUser ? (
    ...
    <main className="">
      <h2 className="text-black font-bold ps-5 pt-2 text-3xl md:text-3xl text-center">👋 Hello {authUser.username} 😊</h2>
      ...
      <div className="max-w-3xl mx-auto mt-5 p-5">
        <div className="bg-white -m-6 p-3 sticky top-0">
          <div className="flex justify-center flex-col items-center">
            ...
            <h1 className="text-3xl md:text-7xl font-bold">To Do's App</h1>
          </div>
        ...
        <div className="my-10">
          {todos.length > 0 &&
            todos.map((todo) => (
              <div
                key={todo.id}
                className="flex items-center justify-between mt-4 text-3xl "
              >
                <div className="flex items-center gap-3">
                  <input
                    id={`todo-${todo.id}`}
                    type="checkbox"
                    className="w-6 h-6 accent-green-400 rounded-lg"
          ...
          {todos.length < 1 && (
            <span className="text-center w-full block text-2xl md:text-5xl font-medium text-gray-400 mt-28">{`🥹 You don't have todo's`}</span>
          )}
        </div>
      </div>
    </main>
